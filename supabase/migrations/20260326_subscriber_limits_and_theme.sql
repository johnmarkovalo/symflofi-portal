-- ============================================================================
-- Migration: Add max_subscribers and theme_customization to license_tiers
-- Date: 2026-03-26
-- Description: Adds per-tier subscriber limits for PPPoE management and
--              theme_customization flag for portal appearance customization.
--              Updates validate_license RPC to return these fields.
-- ============================================================================

-- 1. Add new columns to license_tiers
ALTER TABLE license_tiers
  ADD COLUMN IF NOT EXISTS max_subscribers INTEGER NOT NULL DEFAULT 3,
  ADD COLUMN IF NOT EXISTS theme_customization BOOLEAN NOT NULL DEFAULT false;

-- 2. Set per-tier values
-- Demo: 2 subscribers, 7-day trial (enforced device-side), pppoe_enabled stays true in cloud
-- Lite: PPPoE not available (pppoe_enabled=false), 0 subscribers
-- Pro: Full PPPoE, 50 subscribers, theme customization
-- Enterprise: Unlimited PPPoE and themes
UPDATE license_tiers SET max_subscribers = 2,   theme_customization = false WHERE name = 'demo'       AND product = 'symflofi';
UPDATE license_tiers SET max_subscribers = 0,   theme_customization = false WHERE name = 'lite'       AND product = 'symflofi';
UPDATE license_tiers SET max_subscribers = 50,  theme_customization = true  WHERE name = 'pro'        AND product = 'symflofi';
UPDATE license_tiers SET max_subscribers = -1,  theme_customization = true  WHERE name = 'enterprise' AND product = 'symflofi';

-- 3. Recreate validate_license RPC with maxSubscribers and themeCustomization
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Fallback: build a minimal tier record with demo defaults.
    -- Use a subquery to avoid fragile positional ROW() constructor.
