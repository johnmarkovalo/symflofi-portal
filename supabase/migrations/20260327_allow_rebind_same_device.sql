-- ============================================================================
-- Migration: Allow re-binding license to the same device after unbind
-- Date: 2026-03-27
-- Description: Add p_is_reactivation param to validate_license. Background
--              revalidation (default) still blocks unbound keys so the tablet
--              detects remote unbind. Explicit reactivation (user enters key
--              manually) bypasses the check, allowing same-device re-bind
--              after unbind or factory reset.
-- ============================================================================
...
BEGIN
  ...
  -- 2b. If key was unbound from this device, block background revalidation
  -- but allow explicit reactivation (user manually entering the key again)
  IF v_key.unbound_from_uuid IS NOT NULL AND v_key.unbound_from_uuid = p_machine_uuid AND NOT p_is_reactivation THEN
    RETURN jsonb_build_object('valid', false, 'error', 'license_unbound');
  END IF;
  ...
  -- 10. Bind license key + clear unbound flag
  UPDATE license_keys
